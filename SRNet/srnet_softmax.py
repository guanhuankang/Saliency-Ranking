    ...
    def forward(self, batch_dict, *args, **argw):
        ...
        if self.training:
            ...
            for i in range(n_max):
                ...
                sal_loss += (-torch.log(torch.softmax(sal, dim=1)+1e-6) * q_ans).sum() / (q_ans.sum()+1e-6)
            ...
            for i in range(nq):
                sal = self.gaze_shift(q=q, z=z, qpe=qpe, zpe=zpe, q_vis=q_vis, bbox=pred_bboxes, size=size)  ## B, nq, 1
                sal = torch.softmax(sal, dim=1)  ## B, nq, 1

                sal_max = torch.argmax(sal[:, :, 0], dim=1).long()  ##  B
                q_vis[bs_idx, sal_max, 0] = i+1

                sal_scores = sal[bs_idx, sal_max, 0]  ## B
