    ...
    def forward(self, batch_dict, *args, **argw):
        ...
        if self.training:
            ...
            for i in range(n_max + 1):
                ...
                sal_pos_loss = F.binary_cross_entropy_with_logits(sal, torch.ones_like(q_ans), reduction="none")
                sal_neg_loss = F.binary_cross_entropy_with_logits(sal, torch.zeros_like(q_ans), reduction="none")
                sal_pos_weight = self.cfg.LOSS.SAL_POS
                sal_neg_weight = self.cfg.LOSS.SAL_NEG
                sal_loss += (
                    q_ans * sal_pos_loss * sal_pos_weight + 
                    (1.0 - q_ans) * sal_neg_loss * sal_neg_weight
                ).mean()
