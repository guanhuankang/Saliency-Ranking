...
class CenterShiftBlock(nn.Module):
    ...
    def forward(self, q, qpe, z, zpe, gaze, gaze_emb):
        """
        q, qpe: B, nq, C
...
class CenterSoftMax(nn.Module):
    ...
    def forward(self, q, z, qpe, zpe, q_vis, bbox, size):
        ...
        predictions = [self.head(q)]
        for layer in self.blocks:
            q = layer(q=q, qpe=qpe, z=z, zpe=zpe, gaze=gaze, gaze_emb=gaze_emb)
