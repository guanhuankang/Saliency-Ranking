...
class GazeShiftLayer(nn.Module):
    ...
    def forward(self, q, z, ior):
        q = q + ior
        q = self.norm1(q + self.dropout1(self.self_attn(q=q, k=q, v=q)))
        q = self.norm2(q + self.dropout2(self.q2z_attn(q=q, k=z, v=z)))
        q = self.norm3(q + self.dropout3(self.mlp(q)))
        return q

class PeripheralWOPE(nn.Module):
    @configurable
    def __init__(self, alpha=1.0, sigma=10.0, kernel_size=5, embed_dim=256, num_heads=8, hidden_dim=1024, dropout_attn=0.0, dropout_ffn=0.0, num_blocks=2):
        ...
        self.posture_mlp = nn.Sequential(
        ...
        init_weights_(self.posture_mlp)
        init_weights_(self.saliency_head)
    ...
    def forward(self, q, z, qpe, zpe, q_vis, bbox, size):
        ...
        """
        device = q.device
        current_fixation = torch.stack([
            bb[vis.argmax(), :] if vis.max() > .5 else torch.ones(4, device=device)*0.5
            for bb, vis in zip(bbox, q_vis[:, :, 0])
        ], dim=0).unsqueeze(1)  ## B, 1, 4 (xywh) in [0, 1]

        ## mimic peripheral vision
        gaze_map = self.getGazeMap(current_fixation, size=size)  ## B, 1, h, w
        ...
        ## dx,dy,w,h (B, nq, 4) vector (B, nq, C)
        posture = bbox.clone()
        posture[:, :, 0:2] = posture[:, :, 0:2] - current_fixation[:, :, 0:2]  ## dxdy
        q = q + self.posture_mlp(posture)  ## B, nq, C

        for layer in self.layers:
            q = layer(q=q, z=z, ior=ior)
