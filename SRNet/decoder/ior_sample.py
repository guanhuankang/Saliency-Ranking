...
class IORSample(nn.Module):
    ...
    def sample(self, x, ior_masks):
        ...
        ior_masks = [F.adaptive_max_pool2d(m.unsqueeze(0).unsqueeze(1), (H, W)) for m in ior_masks]
        points = []
        indices = []
        for i in range(num_ior):
            num = (n+1) if i < r else n
            p, idx = self.point_sampler.samplePoints2D(x, num, mask=ior_masks[i].gt(0.5).to(device), indices=True)
            points.append(p)
            indices.append(torch.stack(idx, dim=0))
        points = torch.cat(points, dim=1).to(device) ## 1,k,C
        indices = torch.cat(indices, dim=1).to(device) ## 3,* (0,H~,W~)

        points = points + self.cls_embedding(torch.zeros((1, self.num_points), dtype=torch.long, device=device))
        return points.to(device), indices.to(device)

    def mock(self, x):
        _, C, H, W = x.shape
        device = x.device
        points = self.cls_embedding(torch.ones((1, self.num_points), dtype=torch.long, device=device)) ## 1,k,C
